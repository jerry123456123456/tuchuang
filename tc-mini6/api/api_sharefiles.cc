...
void handleGetRankingFilelist(int start, int count, string &str_json) {
    ...
    */
    ...
    //===2、redis共享文件数量 榜单数量 redis到底有多少数据， 最终依赖mysql
    ...
    //===3、mysql共享文件数量和redis共享文件数量对比，判断是否相等  从mysql拉取共享文件的记录到redis，比较耗时的
    ...
    { //===4、如果不相等，清空redis数据，重新从mysql中导入数据到redis
      //(mysql和redis交互)
        ...
        // b) 从mysql中导入数据到redis
        // sql语句
        // 直接读取所有的不对，应该分页去读取 100条、100条
        strcpy( sql_cmd, "select md5, file_name, pv from share_file_list order by pv desc"); //虽然可以分页排序
        LogInfo("执行: {}", sql_cmd);
