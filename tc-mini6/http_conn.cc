...
void CHttpConn::OnRead() // CHttpConn业务层面的OnRead
{
    ...
    if (http_parser_.IsReadAll()) {
        ...
        } else if (strncmp(url.c_str(), "/api/loginm", 11) == 0) { // 登录
            _HandleLoginMRequest(url, content);
        } else if (strncmp(url.c_str(), "/api/login", 10) == 0) { // 登录
            _HandleLoginRequest(url, content);
        } else if (strncmp(url.c_str(), "/api/myfilesm", 13) == 0) { //获取我的文件数量
            _HandleMyfilesMRequest(url, content);
        } else if (strncmp(url.c_str(), "/api/myfiles", 12) == 0) { //获取我的文件数量
        ...
        }else if (strncmp(url.c_str(), "/api/testm", 10) == 0) { //
            _HandleTestMRequest(url, content);
        }else if (strncmp(url.c_str(), "/api/tests", 10) == 0) { //
            _HandleTestSRequest(url, content);
        }else {
            LogError("\n\n------------------------url unknown, url= {} -----------------------\n", url);
            Close();
        }
...
//多线程处理 
void CHttpConn::_HandleLoginMRequest(string &url, string &post_data) {
    g_thread_pool.Exec(ApiUserLoginM, conn_handle_, post_data);
}
//单线程处理 
void CHttpConn::_HandleLoginRequest(string &url, string &post_data) {
 ...
 //  多线程版本
void CHttpConn::_HandleMyfilesMRequest(string &url, string &post_data) {
    g_thread_pool.Exec(ApiMyfilesM, conn_handle_, url, post_data);
}
...
void CHttpConn::_HandleMyfilesRequest(string &url, string &post_data) {
 ...
 void CHttpConn::_HandleMd5Request(string &url, string &post_data) {
    // 
    string resp_json;
...
void CHttpConn::_HandleUploadRequest(string &url, string &post_data) {
    string resp_json;
    ...
    snprintf(http_body, HTTP_RESPONSE_HTML_MAX, HTTP_RESPONSE_HTML, ulen,
             resp_json.c_str());
    ret = Send((void *)http_body, strlen(http_body)); // 返回值暂时不做处理
    delete[] http_body;
}  

void CHttpConn::_HandleSharepictureRequest(string &url, string &post_data) {
    string resp_json;
    int ret = ApiSharepicture2(url, post_data, resp_json);
...
void CHttpConn::_HandleDealfileRequest(string &url, string &post_data) {
    string str_json;
...
void CHttpConn::_HandleDealsharefileRequest(string &url, string &post_data) {
    string str_json;
...
void CHttpConn::_HandleSharefilesRequest(string &url, string &post_data) {
    string str_json;
...
void CHttpConn::_HandleTestMRequest(string &url, string &post_data) {
   g_thread_pool.Exec(ApiTestM, conn_handle_, post_data);
}
void CHttpConn::_HandleTestSRequest(string &url, string &post_data) {
    string str_json;
    int ret = ApiTestS(post_data, str_json);
    char *szContent = new char[HTTP_RESPONSE_HTML_MAX];
    uint32_t ulen = str_json.length();
    snprintf(szContent, HTTP_RESPONSE_HTML_MAX, HTTP_RESPONSE_HTML, ulen,
             str_json.c_str());
...
void CHttpConn::SendResponseDataList() {
    ...
    while (!s_response_pdu_list.empty()) {
        ...
        s_resp_mutex.unlock();
        CHttpConn *pConn = FindHttpConnByHandle(pResp->conn_uuid); // 该连接有可能已经被释放，如果被释放则返回NULL
