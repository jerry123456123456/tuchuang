...
//分享图片
static void handleSharePicture(const char *user, const char *filemd5,
...
//解析的json包
static int decodePictureListJson(string &str_json, string &user_name, string &token,
                          int &start, int &count) {
    bool res;
    Json::Value root;
    Json::Reader jsonReader;
...
static int encodeCancelPictureJson(int ret, string &str_json) {
    Json::Value root;
    root["code"] = ret;
    Json::FastWriter writer;
    str_json = writer.write(root);
...
//取消分享文件
static void handleCancelSharePicture(const char *user, const char *urlmd5,
                              string &str_json) {
    char sql_cmd[SQL_MAX_LEN] = {0};
