...
bool CResultSet::Next() {
    row_ = mysql_fetch_row(res_); // 检索结果集的下一行,也就是查询的结果的具体数据,行内值的数目由mysql_num_fields(result)给出
    if (row_) {
        return true;
    } else {
        return false;
...
void CDBPool::RelDBConn(CDBConn *pConn) {
    ...
    if (it == free_list_.end()) {
        ...
        LogWarn("RelDBConn failed");  // 不再次回收连接
    }
}
// 遍历检测是否超时未归还
...
int CDBManager::Init() {
    ...
    if(config_file.ParseConf(conf_path_.c_str()) != 0) {
        std::cout << conf_path_ << " no exist\n";
        return -1;
    }
    char *db_instances = config_file.GetConfigName("DBInstances");    //将配置文件里面的主从master和slave存进去
