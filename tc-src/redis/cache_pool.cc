...
int CacheManager::Init() {
    ...
    for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++) {
        ...
        if (!cache_host || !str_cache_port || !str_cache_db ||
            !str_max_conn_cnt) {
            if(!cache_host)
                LogError("not configure cache instance: {}, cache_host is null", pool_name);
            if(!str_cache_port)
                LogError("not configure cache instance: {}, str_cache_port is null", pool_name);
            if(!str_cache_db)
                LogError("not configure cache instance: {}, str_cache_db is null", pool_name);
            if(!str_max_conn_cnt)
                LogError("not configure cache instance: {}, str_max_conn_cnt is null", pool_name);
            return 2;
        }
